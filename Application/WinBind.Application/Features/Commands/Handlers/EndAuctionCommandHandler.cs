...
{
    public class EndAuctionCommandHandler(IRepository<Auction> _auctionRepo, IRepository<AuctionResult> _auctionResultRepo, IRepository<Bid> _bidRepo) : IRequestHandler<EndAuctionCommandRequest, ResponseModel<bool>>
    {
                ...
                if (_auctionRepo.Update(auction))
                {
                    ...
                    if (saveAuctionResponse) // müzayede sonlandırıldı
                    {
                        ...
                        if (await _auctionResultRepo.AddAsync(auctionResult))
                            if (await _auctionResultRepo.SaveChangesAsync())
                            {
